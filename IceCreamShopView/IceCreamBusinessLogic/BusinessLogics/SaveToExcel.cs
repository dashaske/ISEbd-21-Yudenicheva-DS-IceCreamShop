    ...
    class SaveToExcel
    {
        public static void CreateDoc(ExcelInfo info)
        {
            ...
            SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var pc in info.IngredientIceCreams)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                    ...
                    foreach (var component in pc.Ingredients)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
...
spreadsheetDocument.WorkbookPart.AddNewPart<SharedStringTablePart>();
                ...
                uint rowIndex = 2;
                foreach (var pc in info.WareHouseIngredients)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                    ...
                    foreach (var secure in pc.Ingredients)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
